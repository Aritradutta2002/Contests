...
import java.io.*;
/*
 * Problem: EZSPEAK - Easy Pronunciation
 * Difficulty: 1200 (1-star)
 * URL: https://www.codechef.com/practice/course/1-star-difficulty-problems/DIFF1200/problems/EZSPEAK
 *
 * Description:
 *   A word is considered hard to pronounce if it contains 4 or more
 *   consecutive consonants. Given N words, determine how many of them
 *   are hard to pronounce.
 *
 *   Vowels are: a, e, i, o, u (all lowercase).
 *
 * Input:
 *   - First line: T (number of test cases)
 *   - For each test case:
 *     - First line: N (number of words)
 *     - Second line: N space-separated words (lowercase English letters)
 *
 * Output:
 *   - For each test case, output the number of hard-to-pronounce words.
 *
 * Constraints:
 *   1 <= T <= 100
 *   1 <= N <= 100
 ...
 class Easy_Pronunciation {
    ...
    static void solve() throws Exception {
        ...
        for(char c : s.toCharArray()){
            if(c != 'a' || c != 'e' || c != 'i' || c != 'o' || c != 'u') consonantCount++;
            else consonantCount = 0;
            if(consonantCount >= 4) ans = true;
        }
