...
class SetSize {
    ...
    static void solve() throws Exception {
        ...
        int[] uniqueA = getUniqueSortedArray(a);
        int k = uniqueA.length;

        if (k == 1) {
            while (q-- > 0) {
                sb.append(nextLong()).append('\n');
            }
            return;
        }

        long[] sortedGaps = getSortedGaps(uniqueA);
        long[] prefixSums = getPrefixSums(sortedGaps);
    ...
    private static long[] getSortedGaps(int[] uniqueA) {
        ...
        for (int i = 0; i < numGaps; i++) {
            gaps[i] = (long) uniqueA[i + 1] - uniqueA[i];
        }
        Arrays.sort(gaps);
        return gaps;
    }
