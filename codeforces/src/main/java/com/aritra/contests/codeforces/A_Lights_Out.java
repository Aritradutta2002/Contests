...
public class A_Lights_Out {
    ...
    static void solve() throws IOException {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                if (arr[i][j] % 2 != 0) {
                    toggleGrid(grid, i, j);     // Own postion
                    toggleGrid(grid, i - 1, j);  // left adjecent 
                    toggleGrid(grid, i + 1, j); //right adjecent 
                    toggleGrid(grid, i, j - 1); //bottom adjecent 
                    toggleGrid(grid, i, j + 1); //upper adjecent 
                }
            }
        }
        printGrid(grid);
    ...
    static void toggleGrid(int[][] grid, int i, int j) {
        int N = grid.length;
        if (i >= 0 && i < N && j >= 0 && j < N) {
            grid[i][j] ^= 1; // flips 1-> 0 and 0 -> 1
        }
    }
